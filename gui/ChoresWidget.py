...
class ChoresWidget(QWidget):
  ...
  def Init(self):
    ...
    self.Grid.addWidget(QLabel('Reward:', self), nrow, 2, 1, 1, Qt.AlignRight)
  ...
  def AddNewChore(self):
    ...
    self.LineEditPoints.setText('2.50')
  ...
  def SaveChore(self):
    if (self.ListWidget.currentItem()):
      self.DataHandlerObject.EditItem('chores', self.ListWidget.currentItem().ChoreData['uuid'], {'name': self.LineEditName.text(), 'freq': int(self.LineEditFreq.text()), 'priority': int(self.LineEditPrio.text()), 'alast': self.LineEditLastA.text(), 'reward': float(self.LineEditPoints.text())})
    else:
      self.DataHandlerObject.AddNewItem('chores', {'name': self.LineEditName.text(), 'freq': int(self.LineEditFreq.text()), 'priority': int(self.LineEditPrio.text()), 'atimes' : 0, 'alast': self.LineEditLastA.text(), 'reward': float(self.LineEditPoints.text())})
    self.LoadChores()
