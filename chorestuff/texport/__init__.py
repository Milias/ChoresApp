...
@Singleton
class TeXporter:
  def __init__(self, export_path = 'tex/', template_path = 'tex/'):
  ...
  def NewBillDutch(self, bill):
    template = self.Env.get_template('tex/bill-template-dutch.tex')
  ...
  def NewExpensesTable(self, begin_date, end_date):
    template = self.Env.get_template('tex/expenses-table.tex')
