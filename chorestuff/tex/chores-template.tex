  ...
  \begin{tabularx}{\textwidth}{|c|c|X|c|c|}
    ...
    \BLOCK{ for assignment in bundle.assignments }
      \BLOCK{- if assignment.tenant -}
        \VAR{'  '} \VAR{ '%3s' % ('Yes' if assignment.is_tenant_home else 'No') } & \VAR{ '%15s' % tex_escape(assignment.tenant.name) } & \VAR{ '%20s' % (tex_escape(assignment.chore.name) if assignment.chore else '(None)') } & \phantom{---------------} & \\[0.25cm] \hline \VAR{ '\n    ' }
      \BLOCK{- endif -}
    \BLOCK{ endfor }
    \BLOCK{- for assignment in bundle.assignments -}
